...
import random
from maze_generator.helper import event
...
class Cell(object):
    ...
    def get_wall_mask(self):
        return (
            [not self.exists_and_is_linked(n) for n in self._neighbors]
            if self.has_any_link()
            else [False] * len(self._neighbors)
        )
...
class CellOver(Cell):
    ...
    def neighbors_copy(
        self,
    ):  # Keep this for method patching using Kruskal's algorithm and weave maze until I know how to do it cleanly.
        if self.neighbors_over is None:
            self.neighbors_over = [None] * len(self._NEIGHBORS_RETURN)
            for i, neighbor in enumerate(self._neighbors):
                self.neighbors_over[i] = neighbor.neighbor(i) if self.can_tunnel_towards(i) else None
    ...
    def can_host_under_vertical_psg(self):
        return (
            self.is_linked_and_cell_over(1)
            and self.is_linked_and_cell_over(3)
            and not self.is_linked_and_cell_over(0)
            and not self.is_linked_and_cell_over(2)
        )

    def can_host_under_horiz_psg(self):
        return (
            not self.is_linked_and_cell_over(1)
            and not self.is_linked_and_cell_over(3)
            and self.is_linked_and_cell_over(0)
            and self.is_linked_and_cell_over(2)
        )
...
class CellUnder(Cell):
    ...
    def can_host_under_vertical_psg(self):
        return (self.neighbor(cst.WEST) and type(self.neighbor(cst.WEST)) is not CellUnder) or (
            self.neighbor(cst.EAST) and type(self.neighbor(cst.EAST)) is not CellUnder
        )

    def can_host_under_horiz_psg(self):
        return (self.neighbor(cst.NORTH) and type(self.neighbor(cst.NORTH)) is not CellUnder) or (
            self.neighbor(cst.SOUTH) and type(self.neighbor(cst.SOUTH)) is not CellUnder
        )
